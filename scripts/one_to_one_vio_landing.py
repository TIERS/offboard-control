...
class LandingControl() :
    ...
    def __init__(self) :
        ...
        self.offboard_state_topic = rospy.get_param("~offboard_state_topic", "/uav/safe_offboard_state")
        ...
        # Wait until we are flying (offboard_state) before trying to do anything!
        sys.stdout.write("Waiting for offboard state to go from '{}' to 'flying'...".format(self.offboard_state))
        sys.stdout.flush()
        waiting_cnt = 0.1
        while not self.pos.header.stamp :
            sys.stdout.write("\rWaiting for offboard state to go from '{}' to 'flying'... {}s".format(self.offboard_state, waiting_cnt))
            sys.stdout.flush()
            waiting_cnt += 0.1
            time.sleep(0.1)
    ...
    def run(self) :
        ...
        while not self.publishing_objective :
